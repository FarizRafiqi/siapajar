...
export async function getCurriculumContext(
  ...
  const ids = sequence.items
    ...
    .filter(Boolean)
  const objectives = ids.length
    ? await LearningObjective.query().whereIn('id', ids).preload('indicators')
    : []
  ...
  return {
    ...
    objectives: ids
      .map((id) => byId.get(id))
      .filter((objective): objective is LearningObjective => Boolean(objective))
      .map((objective) => ({
        id: objective.id,
        code: objective.code,
