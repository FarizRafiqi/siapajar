...
class AiQueueService {
  async enqueueAiJson<T>(options: { userId?: number; combo: string; systemPrompt: string; userPrompt: string; priority?: 'high' | 'normal' | 'low'; timeoutMs?: number }): Promise<T> {
    if (!options.userId) throw new Error('AI jobs require an authenticated user')
    const owner = await User.findOrFail(options.userId)
    const jobKey = createHash('sha256').update(`${options.userId}:${options.combo}:${options.systemPrompt}:${options.userPrompt}`).digest('hex')
    ...
    if (job.status !== 'completed') {
      const reserved = await reserveUsage(owner, 'ai_generation_monthly', jobKey, 1, { combo: options.combo })
      try {
        await GenerateAiJson.dispatch({ jobKey, userId: options.userId, combo: options.combo, systemPrompt: options.systemPrompt, userPrompt: options.userPrompt, timeoutMs: options.timeoutMs }).dedup({ id: jobKey, ttl: '5m' })
        await auditService.record({ actorId: options.userId, action: 'ai.generate.queued', entityType: 'ai_job', entityId: job.id, metadata: { combo: options.combo, quotaReserved: reserved } })
      } catch (error) {
  ...
  getStats() {
    return { driver: env.get('QUEUE_DRIVER', 'redis'), maxConcurrency: env.get('AI_QUEUE_MAX_CONCURRENCY', 10), redisHost: env.get('REDIS_HOST') || '127.0.0.1' }
  }
}
