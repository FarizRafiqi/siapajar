...
function ToolbarButton({ active, disabled, label, onClick, children }: Readonly<{ active?: boolean; disabled?: boolean; label: string; onClick: () => void; children: React.ReactNode }>) {
  return (
    <button
      type="button"
      title={label}
      ...
      disabled={disabled}
      onClick={onClick}
      className={`rounded p-1.5 transition-colors disabled:opacity-40 ${active ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300' : 'text-neutral-600 hover:bg-neutral-100 dark:text-neutral-300 dark:hover:bg-neutral-800'}`}
    >
      {children}
    </button>
  )
...
export default function RichTextEditor({ value, onChange, placeholder, disabled = false, minHeight = '8rem' }: RichTextEditorProps) {
  ...
  if (!editor) return <div className="rounded-lg border border-neutral-300 bg-neutral-50 p-3 text-sm text-neutral-500">Memuat editor...</div>
  ...
  return (
    <div className={`overflow-hidden rounded-lg border border-neutral-300 bg-white dark:border-neutral-600 dark:bg-neutral-800 ${disabled ? 'opacity-70' : ''}`}>
      {!disabled && (
        <div className="flex flex-wrap items-center gap-1 border-b border-neutral-200 bg-neutral-50 p-1 dark:border-neutral-700 dark:bg-neutral-900">
          ...
          <ToolbarButton label="Link" active={editor.isActive('link')} onClick={setLink}><LinkIcon className="h-4 w-4" /></ToolbarButton>
          <span className="mx-1 h-5 w-px bg-neutral-200 dark:bg-neutral-700" />
          <select aria-label="Alignment" value={editor.isActive({ textAlign: 'center' }) ? 'center' : editor.isActive({ textAlign: 'right' }) ? 'right' : editor.isActive({ textAlign: 'justify' }) ? 'justify' : 'left'} onChange={(event) => editor.chain().focus().setTextAlign(event.target.value as 'left' | 'center' | 'right' | 'justify').run()} className="rounded border-0 bg-transparent px-1 py-1 text-xs text-neutral-600 outline-none dark:text-neutral-300">
      ...
      <div style={{ minHeight }} className="[&_.ProseMirror_p.is-editor-empty:first-child::before]:float-left [&_.ProseMirror_p.is-editor-empty:first-child::before]:pointer-events-none [&_.ProseMirror_p.is-editor-empty:first-child::before]:h-0 [&_.ProseMirror_p.is-editor-empty:first-child::before]:text-neutral-400 [&_.ProseMirror_p.is-editor-empty:first-child::before]:content-[attr(data-placeholder)]">
