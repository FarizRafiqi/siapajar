...
export default function PaudAssessmentsIndex({ assessments, classes, typeLabels }: PaudAssessmentsIndexProps) {
  ...
  const handleAdd = () => {
    ...
    if (data.type === 'checklist') {
      ...
      content = { photoDescription: data.photoDescription, description: data.description, analysis: data.analysis }
  ...
  const filteredAssessments = filterType === 'all' ? assessments : assessments.filter((a) => a.type === filterType)
    ...
    >
      ...
      <div className="space-y-6">
        ...
        {!hasClasses && (
          <div className="rounded-lg border border-amber-300 bg-amber-50 p-4 dark:border-amber-800 dark:bg-amber-900/20">
            <p className="text-sm font-medium text-amber-800 dark:text-amber-300">Buat kelompok dulu sebelum mencatat asesmen.</p>
            <Link
              href="/classes"
        ...
        {hasClasses && !hasStudents && (
          <div className="rounded-lg border border-amber-300 bg-amber-50 p-4 dark:border-amber-800 dark:bg-amber-900/20">
            <p className="text-sm font-medium text-amber-800 dark:text-amber-300">Tambah siswa dulu sebelum mencatat asesmen.</p>
          </div>
        )}
        ...
        {filteredAssessments.length === 0 ? (
          <div className="rounded-xl border border-dashed border-neutral-300 py-12 text-center dark:border-neutral-700">
            ...
            <h3 className="mt-4 text-lg font-medium text-neutral-900 dark:text-white">Belum ada asesmen</h3>
            <p className="mt-2 text-neutral-500 dark:text-neutral-400">Catat asesmen pertama untuk anak didik Anda</p>
          </div>
        ) : (
                  ...
                  <div className="flex items-start justify-between">
                    ...
                    <div className="mt-2 text-sm text-neutral-700 dark:text-neutral-300">
                      ...
                      {item.type === 'anecdotal_note' && (
                        <p>
                          <strong>Latar:</strong> {item.content.context} — <strong>Perilaku:</strong>{' '}
                          {item.content.behavior} — <strong>Analisis:</strong> {item.content.analysis}
                        </p>
                      )}
                      {item.type === 'work_sample' && (
                        <p>
                          <strong>{item.content.photoDescription}</strong> — {item.content.description}
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          ...
          >
            <h3 className="mb-4 text-lg font-semibold text-neutral-900 dark:text-white">Catat Asesmen</h3>
            <div className="space-y-4">
              ...
              <div>
                <label htmlFor="studentId" className="mb-1 block text-sm font-medium text-neutral-700 dark:text-neutral-300">
                  Anak
                </label>
                <select
                  id="studentId"
              ...
              {data.type === 'checklist' && (
                <>
                  <div>
                    <label htmlFor="indicators" className="mb-1 block text-sm font-medium text-neutral-700 dark:text-neutral-300">
              ...
              {data.type === 'anecdotal_note' && (
                <>
                  <div>
                    <label htmlFor="context" className="mb-1 block text-sm font-medium text-neutral-700 dark:text-neutral-300">
                      Latar Kejadian
                    </label>
                    <input
                      id="context"
                  ...
                  <div>
                    <label htmlFor="behavior" className="mb-1 block text-sm font-medium text-neutral-700 dark:text-neutral-300">
                  ...
                  <div>
                    <label htmlFor="analysis" className="mb-1 block text-sm font-medium text-neutral-700 dark:text-neutral-300">
                      Analisis
                    </label>
                    <input
                      id="analysis"
              ...
              {data.type === 'work_sample' && (
                <>
                  <div>
                    <label htmlFor="photoDescription" className="mb-1 block text-sm font-medium text-neutral-700 dark:text-neutral-300">
                      Judul Hasil Karya
                    </label>
                    <input
                      id="photoDescription"
                  ...
                  <div>
                    <label htmlFor="workAnalysis" className="mb-1 block text-sm font-medium text-neutral-700 dark:text-neutral-300">
              ...
              {data.type === 'photo_series' && (
                <>
                  <div>
                    <label htmlFor="activity" className="mb-1 block text-sm font-medium text-neutral-700 dark:text-neutral-300">
                      Kegiatan
                    </label>
                    <input
                      id="activity"
                  ...
                  <div>
                    <label htmlFor="narrative" className="mb-1 block text-sm font-medium text-neutral-700 dark:text-neutral-300">
      ...
      {deletingAssessment && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          ...
          >
            <h3 className="mb-2 text-lg font-semibold text-neutral-900 dark:text-white">Hapus Asesmen?</h3>
            <p className="text-neutral-600 dark:text-neutral-400">Asesmen ini akan dihapus secara permanen.</p>
            <div className="mt-6 flex gap-3">
              <button
