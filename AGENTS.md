...
- **Background jobs follow the same boundary.** Jobs may delegate to services; database reads and writes belong in services only when simple under the exception above, otherwise in repositories. Pure prompt construction, parsing, rendering, export formatting, and other non-persistence code do not need a repository.
